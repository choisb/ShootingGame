    ...
    : Damage(100.f)
{
    ...
    OverlapSphere = CreateDefaultSubobject<USphereComponent>(TEXT("Overlap Shpere"));
...
void AExplosive::BulletHit_Implementation(FHitResult HitResult, AActor* Shooter, AController* ShooterController)
{
    ...
    {
        const float distance = FVector(Actor->GetActorLocation() - GetActorLocation()).Size();
        if(distance < OverlapSphere->GetScaledSphereRadius())
